...
{
    path_abs = ros::package::getPath("bbblue_navigation") + path_local;
...
int main(int argc, char** argv)
{
    ...
    ros::Publisher pubWaypointNodeEnded = n.advertise<std_msgs::Bool>("/bbblue_navigation/waypoint_following_status", 100);
    ...
    ros::param::get("/bbblue_navigation/coordinates_file", path_local);
    ...
    for(iter = waypointVect.begin(); iter < waypointVect.end(); iter++)
    {
        ...
        {
            ROS_INFO("bbblue_bot has reached its goal!");
            //switch to next waypoint and repeat
        }
        else
        {
            ROS_ERROR("bbblue_bot was unable to reach its goal. GPS Waypoint unreachable.");
    ...
    ROS_INFO("bbblue_bot has reached all of its goals!!!\n");
